...
// .wrangler/tmp/bundle-iCWd4O/checked-fetch.js
...
// .wrangler/tmp/bundle-iCWd4O/strip-cf-connecting-ip-header.js
...
var getMockTailoredResume = /* @__PURE__ */ __name((resumeText, jd, qaAnswers) => {
  return {
    name: "Alex Morgan",
...
app.post("/api/analyze", async (c) => {
  ...
  if (resumeText.length > 5e4 || jobDescription.length > 5e4) {
...
app.post("/api/query-resume", async (c) => {
  ...
  if (resumeText.length > 5e4 || question.length > 1e4) {
    return c.json({ success: false, message: "Payload content size exceeds safe boundaries." }, 400);
  }
  let usageCount = 0;
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
app.post("/api/generate", async (c) => {
  ...
  if (resumeText.length > 5e4 || jobDescription.length > 5e4 || saveName.length > 200) {
    return c.json({ success: false, message: "Payload content size exceeds safe boundaries." }, 400);
  }
  let usageCount = 0;
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
// .wrangler/tmp/bundle-iCWd4O/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-iCWd4O/middleware-loader.entry.ts
